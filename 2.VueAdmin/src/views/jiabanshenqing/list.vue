	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('jiabanshenqing','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('jiabanshenqing','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('jiabanshenqing','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('jiabanshenqing','删除')">删除</el-button>
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('jiabanshenqing','加班统计')">加班统计</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				ref="table"
				v-if="btnAuth('jiabanshenqing','查看')"
				:data="list"
				...
				<el-table-column label="审核" v-if="btnAuth('jiabanshenqing','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('jiabanshenqing','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'加班统计'" width="70%">
...
<script setup>
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			...
			context?.$http({
				url: `jiabanshenqing/group/xingming`,
				method: 'get'
			}).then(res=>{
	...
	const approvalClick = (row) => {
		let params = {
			...
			jiabankaishishijian: row.jiabankaishishijian,
			jiabanjieshushijian: row.jiabanjieshushijian,
			jiabanyuanyin: row.jiabanyuanyin,
			jiabanfaqiriqi: row.jiabanfaqiriqi,
			shhf: row.shhf,
			sfsh: row.sfsh,
		}
		nextTick(() => {
