	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('waichushenqing','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('waichushenqing','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('waichushenqing','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('waichushenqing','删除')">删除</el-button>
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('waichushenqing','外出统计')">外出统计</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				ref="table"
				v-if="btnAuth('waichushenqing','查看')"
				:data="list"
				...
				<el-table-column label="审核" v-if="btnAuth('waichushenqing','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('waichushenqing','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'外出统计'" width="70%">
			<div  id="xingmingEchart1" style="width:100%;height:600px;"></div>
...
<script setup>
	...
	//基础信息
	const tableName = 'waichushenqing'
	const formName = '外出申请'
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var xingmingEchart1 = echarts.init(document.getElementById("xingmingEchart1"),'macarons');
			context?.$http({
				url: `waichushenqing/group/xingming`,
				method: 'get'
			}).then(res=>{
				...
				// 使用刚指定的配置项和数据显示图表。
				xingmingEchart1.setOption(option);
				  //根据窗口的大小变动图表
				window.onresize = function() {
				    xingmingEchart1.resize();
				};
			})
		})
	}
	...
	const approvalClick = (row) => {
		let params = {
			...
			bumenfuzeren: row.bumenfuzeren,
			bumenlianxidianhua: row.bumenlianxidianhua,
			waichukaishishijian: row.waichukaishishijian,
			waichujieshushijian: row.waichujieshushijian,
			waichufaqiriqi: row.waichufaqiriqi,
			shhf: row.shhf,
			sfsh: row.sfsh,
		}
		nextTick(() => {
