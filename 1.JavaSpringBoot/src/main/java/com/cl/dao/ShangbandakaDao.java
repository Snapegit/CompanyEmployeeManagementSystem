package com.cl.dao;

import com.cl.entity.ShangbandakaEntity;
import com.baomidou.mybatisplus.mapper.BaseMapper;
import java.util.List;
import java.util.Map;
import com.baomidou.mybatisplus.mapper.Wrapper;
import com.baomidou.mybatisplus.plugins.pagination.Pagination;

import org.apache.ibatis.annotations.Param;
import com.cl.entity.view.ShangbandakaView;


/**
 * 上班打卡
 * 
 * @author 
 * @email 
 * @date 2024-03-03 16:25:42
 */
public interface ShangbandakaDao extends BaseMapper<ShangbandakaEntity> {
	
	List<ShangbandakaView> selectListView(@Param("ew") Wrapper<ShangbandakaEntity> wrapper);

	List<ShangbandakaView> selectListView(Pagination page,@Param("ew") Wrapper<ShangbandakaEntity> wrapper);
	
	ShangbandakaView selectView(@Param("ew") Wrapper<ShangbandakaEntity> wrapper);
	

    List<Map<String, Object>> selectValue(@Param("params") Map<String, Object> params,@Param("ew") Wrapper<ShangbandakaEntity> wrapper);

    List<Map<String, Object>> selectTimeStatValue(@Param("params") Map<String, Object> params,@Param("ew") Wrapper<ShangbandakaEntity> wrapper);

    List<Map<String, Object>> selectGroup(@Param("params") Map<String, Object> params,@Param("ew") Wrapper<ShangbandakaEntity> wrapper);



}
